...
void ElectronicComponentDetector::move(char dir) {
...
void ElectronicComponentDetector::decode(char pad, char* code, char debug) {
  ...
  switch(pad) {
    ...
    case 0:
      ...
      /*
      digitalWrite(SW_CONTROL_A_1,HIGH);
      digitalWrite(SW_CONTROL_B_2,LOW); //Tone Detector Connected
      digitalWrite(SW_CONTROL_B_4,LOW); //Connect Inductor

      start_timepoint = micros();

      digitalWrite(SW_CONTROL_A_1,LOW);
      if(debug == 1) {
        while(micros() < (start_timepoint + 2000)) {
          if(!digitalRead(CAP_DETECTED)) {
            delayMicroseconds(200);
            if(!digitalRead(CAP_DETECTED)) {Serial.println("CAP DETECTED"); break;}
          }
        }
      }

      digitalWrite(SW_CONTROL_B_2,HIGH); //Tone Detector Disconnect
      digitalWrite(SW_CONTROL_B_4,HIGH); //Disconnect Inductor
      */
